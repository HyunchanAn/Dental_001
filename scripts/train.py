...
if torch.cuda.is_available():
    ...
    DEVICE = 'cpu'
LEARNING_RATE = 1e-4
BATCH_SIZE = 16 # RTX 5080 can handle this for 512x512 images
EPOCHS = 200
HEATMAP_OUTPUT_SIZE = (256, 256) # Upgraded from 128x128 for higher precision
HEATMAP_SIGMA = 3 # Increased sigma slightly for higher resolution heatmap
...
def main():
    ...
    for epoch in range(EPOCHS):
        if not fine_tuning_activated and epoch >= FINE_TUNE_EPOCH:
            ...
            early_stop_counter = 0 # Reset counter when fine-tuning starts!
            print(f"Model backbone unfrozen. Optimizer reset with new learning rate: {FINE_TUNE_LR}")
            print(f"Early stopping counter reset for the fine-tuning phase.")
