...
class UNetHeatmapModel(nn.Module):
    def __init__(self, num_landmarks=NUM_LANDMARKS, pretrained=True):
        ...
        self.enc0 = nn.Sequential(resnet.conv1, resnet.bn1, resnet.relu) # Output: 64 channels, 1/2 size (256x256 for 512 input)
        ...
        self.up_conv1 = self._upsample_block(256 + 256, 128) # Extra upsampling to reach 256x256
    ...
    def forward(self, x):
        ...
        u4 = self.up_conv4(x4)                           # 1/16
        u3 = self.up_conv3(torch.cat([u4, x3], 1))       # 1/8
        u2 = self.up_conv2(torch.cat([u3, x2], 1))       # 1/4
